...
#define VEC_Xf	Vec8f
#define FLOATS_IN_CACHE_LINE 16
#define THRESHOLD (FLOATS_IN_CACHE_LINE * 1)
...
void rec_reset_scalar(int low ,int high) {
  if((high-low) > THRESHOLD) {
    int mid = (high+low)/2;
    Task_handle* task = argolib::fork([=]() {
      rec_reset_scalar(low, mid);
    });
    rec_reset_scalar(mid, high);
    argolib::join(task);
  } else {
...
//Instructs the compiler to turn-off vectorization for this method
__attribute__((optimize("no-tree-vectorize")))
void rec_mul_scalar(int low ,int high) {
  if((high-low) > THRESHOLD) {
    int mid = (high+low)/2;
    Task_handle* task = argolib::fork([=]() {
      rec_mul_scalar(low, mid);
    });
    rec_mul_scalar(mid, high);
    argolib::join(task);
  } else {
    for(int i=low; i<high; i++) {
...
void par_mul_scalar() {
  rec_reset_scalar(0, size);
  rec_mul_scalar(0, size);
}

void rec_reset_vector(int low ,int high) {
  if((high-low) > THRESHOLD) {
    int mid = (high+low)/2;
    Task_handle* task = argolib::fork([=]() {
      rec_reset_vector(low, mid);
    });
    rec_reset_vector(mid, high);
    argolib::join(task);
  } else {
    for(int i=low; i<high; i++) {
      VEC_Xf _A(1.0f);
...
void rec_mul_vector(int low ,int high) {
  if((high-low) > THRESHOLD) {
    int mid = (high+low)/2;
    Task_handle* task = argolib::fork([=]() {
      rec_mul_vector(low, mid);
    });
    rec_mul_vector(mid, high);
    argolib::join(task);
  } else {
    for(int i=low; i<high; i++) {
      for(int k=0; k<size; k++) {
...
void par_mul_vector() {
  rec_reset_vector(0, size);
  rec_mul_vector(0, size);
}
